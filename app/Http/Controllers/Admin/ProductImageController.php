...
class ProductImageController extends Controller
{
    ...
    public function upload($primaryImage, $images)
    {
        ...
        $primaryImage->move(public_path(env('PRODUCT_IMAGES_UPLOAD_PATH_SERVER')), $fileNamePrimaryImage);

        $fileNameImages = [];
        foreach ($images as $image) {
            $fileNameImage = generateFileName($image->getClientOriginalName());

            $image->move(public_path(env('PRODUCT_IMAGES_UPLOAD_PATH_SERVER')), $fileNameImage);
    ...
    public function add(Request $request, Product $product)
    {
        ...
        try {
            ...
            if ($request->has('primary_image')) {
                ...
                $request->primary_image->move(public_path(env('PRODUCT_IMAGES_UPLOAD_PATH_SERVER')), $fileNamePrimaryImage);
