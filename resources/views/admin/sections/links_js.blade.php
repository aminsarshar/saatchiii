<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
<script src="{{ asset('/admin/vendors/js/jquery-3.3.1.min.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/bootstrap.min.js') }}"></script>
<script src="{{ asset('admin/js/jquery.md.bootstrap.datetimepicker.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/core/popper.min.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/perfect-scrollbar.jquery.min.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/prism.min.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/jquery.matchHeight-min.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/pace/pace.min.js') }}"></script>

<script src="{{ asset('/admin/vendors/js/chartist.min.js') }}"></script>
<script src="{{ asset('/admin/js/app-sidebar.js') }}"></script>
<script src="{{ '/admin/js/notification-sidebar.js' }}"></script>
<script src="{{ asset('/admin/js/customizer.js') }}"></script>
<script src="{{ asset('/admin/js/dashboard-ecommerce.js') }}"></script>
<script src="{{ asset('/admin/vendors/js/jqBootstrapValidation.js') }}"></script>
<script src="{{ asset('/admin/js/form-validation.js') }}"></script>
<script src="{{ asset('admin/js/jquery.czMore-latest.js') }}"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap-select@1.13.14/dist/js/bootstrap-select.min.js"></script>
<script src="{{ asset('admin/js/ckeditor.js') }}"></script>
jquery.czMore-latest
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
@include('sweetalert::alert', ['cdn' => 'https://cdn.jsdelivr.net/npm/sweetalert2@9'])
<script src="https://cdn.ckeditor.com/4.21.0/full-all/ckeditor.js"></script>
